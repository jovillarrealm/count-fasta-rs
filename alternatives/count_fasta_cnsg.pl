...
#Cuenta numero de bases, Numero de secuencias y saca el N50
...
foreach my $id (keys %seq) {
    ...
    $NCount += ($seq{$id} =~ tr/nN/nN/); # modificacion contar N
...
my @large = sort { $b <=> $a} @seqLengths; #modificacion 
...
my $avr = sprintf("%i",($totalLength/$n));

my $larg = shift(@large); # modificacion

my $short = pop(@large);#modificacion
...
print "Average contig length is:\t".$avr." bp","\n";

print "Largest contig:\t\t\t".$larg." bp","\n";

print "Shortest contig:\t\t".$short." bp","\n";
